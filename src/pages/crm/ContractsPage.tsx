...
const STATUSES = ["draft", "sent", "signed", "cancelled"] as const;
...
const ContractsPage = () => {
  ...
  const [form, setForm] = useState({ quote_id: "", file_url: "", status: "draft" as string });
  ...
  const openForm = (row?: any) => {
    setEditing(row ?? null);
    setForm({ quote_id: row?.quote_id ?? "", file_url: row?.file_url ?? "", status: row?.status ?? "draft" });
    setFormOpen(true);
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const op = editing?.id ? supabase.from("contracts").update(form).eq("id", editing.id) : supabase.from("contracts").insert(form);
