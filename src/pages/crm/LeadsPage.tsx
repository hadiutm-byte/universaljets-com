...
const STATUSES = ["new", "contacted", "quote_sent", "negotiation", "confirmed", "lost"] as const;
...
const LeadsPage = () => {
  ...
  const [form, setForm] = useState({ client_id: "", status: "new" as string, source: "" });
  ...
  useEffect(() => { fetch(); fetchClients(); }, [fetch, fetchClients]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const op = editing?.id
      ? supabase.from("leads").update(form).eq("id", editing.id)
      : supabase.from("leads").insert(form);
    const { error } = await op;
    if (error) toast.error(error.message);
    else { toast.success(editing?.id ? "Updated" : "Created"); fetch(); setFormOpen(false); }
  };
  ...
  const selectClass = "w-full bg-secondary/50 rounded-lg px-3 py-2.5 text-[13px] text-foreground font-light focus:outline-none focus:ring-1 focus:ring-primary/20 transition-all border border-border/20";
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="bg-card border-border/30 max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-2">
            ...
            <button type="submit"
              className="w-full py-3 bg-gradient-gold text-primary-foreground text-[9px] tracking-[0.25em] uppercase font-medium rounded-lg hover:shadow-[0_0_30px_-8px_hsla(38,52%,50%,0.45)] transition-all duration-500">
              Save Lead
            </button>
          </form>
        </DialogContent>
