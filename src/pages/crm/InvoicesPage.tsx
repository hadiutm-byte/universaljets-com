...
const InvoicesPage = () => {
  ...
  const [form, setForm] = useState({ contract_id: "", amount: "", status: "pending" as string, due_date: "" });
  ...
  const openForm = (row?: any) => {
    ...
    setForm({
      ...
      status: row?.status ?? "pending", due_date: row?.due_date ? new Date(row.due_date).toISOString().slice(0, 10) : "",
    });
    setFormOpen(true);
  };
