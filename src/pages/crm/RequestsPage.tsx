...
const STATUSES = ["pending", "quoted", "confirmed", "cancelled"] as const;
...
const RequestsPage = () => {
  ...
  const [form, setForm] = useState({ client_id: "", departure: "", destination: "", date: "", passengers: 1, notes: "", status: "pending" as string });
  ...
  const openForm = (row?: any) => {
    ...
    setForm({
      ...
      passengers: row?.passengers ?? 1, notes: row?.notes ?? "", status: row?.status ?? "pending",
    });
    setFormOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const payload = { ...form, passengers: Number(form.passengers), date: form.date || null };
    const op = editing?.id
