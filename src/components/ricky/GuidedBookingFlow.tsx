        ...
        <div className="flex gap-1">
          {stepConfig.map((_, i) => (
            <div
              ...
              style={{
                background: i <= step
                  ? "linear-gradient(90deg, hsla(43,85%,58%,0.8), hsla(38,52%,65%,0.6))"
                  : "hsla(0,0%,100%,0.06)",
              }}
            />
          ))}
          ...
          {step === 1 && (
            <motion.div key="route" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.4 }} className="space-y-1">
              ...
              <motion.button
                ...
                onClick={next}
                className="w-full mt-3 py-3 bg-gradient-gold rounded-xl text-primary-foreground text-[10px] tracking-[0.2em] uppercase font-medium disabled:opacity-25 disabled:cursor-not-allowed cursor-pointer transition-all hover:shadow-[0_0_25px_-5px_hsla(43,85%,58%,0.4)]"
              >
                Continue
              </motion.button>
            </motion.div>
