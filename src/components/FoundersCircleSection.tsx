...
import { Crown, Plane, UserCheck, TrendingDown, Tag, Gift, Star, Check } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { toast } from "sonner";
import { Link } from "react-router-dom";
import { LegalConsent, PremiumSubmitButton, ConfidentialityNotice } from "@/components/forms/PremiumFormComponents";
...
const FoundersCircleSection = () => {
  ...
  const canSubmit = email && name && termsAccepted;
      ...
      <div className="container mx-auto px-8 relative z-10">
        <div className="max-w-5xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="text-center mb-20">
            <p className="text-[11px] tracking-[0.4em] uppercase text-primary mb-5 font-medium">Membership</p>
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-24">
            {membershipTiers.map((tier, i) => (
              <motion.div key={tier.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.6 }}
                className={`relative rounded-2xl overflow-hidden transition-all duration-500 group ${tier.accent ? "shadow-[0_20px_60px_-15px_hsla(0,0%,0%,0.1)] scale-[1.02]" : "hover:shadow-[0_16px_40px_-12px_hsla(0,0%,0%,0.08)]"}`}>
                {tier.accent && <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-gold z-10" />}
                <div className={`px-8 pt-9 pb-6 ${tier.accent ? "bg-gradient-to-b from-charcoal to-charcoal-deep" : "bg-muted"}`}>
                  {tier.accent && <span className="inline-block px-3 py-1 bg-primary/15 text-primary text-[8px] tracking-[0.25em] uppercase font-medium rounded-full mb-5">Most Popular</span>}
                  <p className={`text-[10px] tracking-[0.35em] uppercase font-medium mb-1.5 ${tier.accent ? "text-primary/70" : "text-muted-foreground"}`}>{tier.tagline}</p>
                  <h3 className={`font-display text-2xl font-semibold ${tier.accent ? "text-white" : "text-foreground"}`}>{tier.name}</h3>
                  <p className={`text-[13px] font-light leading-relaxed mt-3 ${tier.accent ? "text-white/60" : "text-muted-foreground"}`}>{tier.desc}</p>
                </div>
                <div className="bg-card border border-t-0 border-border px-8 py-7 rounded-b-2xl">
                  ...
                  <Link to="/contact"
                    className={`block text-center py-3.5 text-[10px] tracking-[0.2em] uppercase font-medium rounded-xl transition-all duration-500 ${
                      tier.accent ? "bg-gradient-gold text-primary-foreground hover:shadow-[0_0_30px_-8px_hsla(45,79%,46%,0.4)] btn-luxury" : "border border-border text-foreground/60 hover:border-primary/30 hover:text-primary"
                    }`}>
          ...
          {/* CTA form */}
          <motion.div id="membership-cta" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}
            ...
            {submitted ? (
              ...
              <div className="space-y-4">
                <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
                  <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Full name" required maxLength={100}
                  ...
                  <button type="submit" disabled={loading || !canSubmit}
                    className="px-8 py-3.5 bg-gradient-gold text-primary-foreground text-[10px] tracking-[0.2em] uppercase font-medium rounded-lg transition-all duration-500 hover:shadow-[0_0_30px_-8px_hsla(45,79%,46%,0.4)] disabled:opacity-40 whitespace-nowrap btn-luxury">
                ...
                <label className="flex items-start justify-center gap-2.5 cursor-pointer">
                  ...
                  <div className={`mt-0.5 w-[16px] h-[16px] rounded border-[1.5px] flex items-center justify-center flex-shrink-0 transition-all duration-300 ${termsAccepted ? "bg-primary border-primary" : "border-foreground/15 bg-muted/40"}`}>
                    {termsAccepted && <Check className="w-2.5 h-2.5 text-primary-foreground" strokeWidth={2.5} />}
                  </div>
                  <span className="text-[11px] text-foreground/40 font-light leading-[1.7] text-left">
                    I agree to the{" "}
                    <Link to="/terms" className="text-primary/60 hover:text-primary underline underline-offset-2 transition-colors">Terms & Conditions</Link>
                    {" "}and{" "}
                    <Link to="/privacy" className="text-primary/60 hover:text-primary underline underline-offset-2 transition-colors">Privacy Policy</Link>.
                  </span>
                </label>
              </div>
            )}
